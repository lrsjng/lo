...
const is_inp_el = el => INPUT_ELS.includes(el.nodeName.toLowerCase());

const has_len = x => x && is_type_of(x.length, 'number');
const to_arr = x => Array.isArray(x) ? x : has_len(x) ? Array.from(x) : [];
const for_each = (x, fn) => to_arr(x).forEach(fn);

const on = (el, type, fn) => el.addEventListener(type, fn);
const off = (el, type, fn) => el.removeEventListener(type, fn);

const table_log = console.table || console.log;
...
binder.prototype = {
    ...
    log() {
        const plain = {};
        Object.keys(this._bdgs).forEach(name => {
        ...
        if (ensure_name(this, name)) {
            ...
            for_each(els, el => {
                if (!bdg.els.includes(el)) {
                    bdg.els.push(el);
                    if (is_inp_el(el)) {
                        on(el, 'input', () => {bdg.set(el.value);});
                        on(el, 'change', () => {bdg.set(el.value);});
                    }
                }
            });
        }
    ...
    collect(containerEl, split) {
        ...
        const els = query_all(`[${ATTR_KEY}]`, containerEl);
