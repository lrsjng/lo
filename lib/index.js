((root, factory) => {
    const is_obj = x => typeof x === 'object';
    ...
    const flat = x => x.reduce((res, xi) => res.concat(Array.isArray(xi) ? flat(xi) : xi), []);
    ...
    dom.prototype = {
        ...
        find(selector) {
            return dom(flat(this.map(el => query_all(selector, el))));
        },
    ...
    const find_all_text_nodes = node => {
        const textnodes = flat(Array.from(node.childNodes, n => find_all_text_nodes(n)));
