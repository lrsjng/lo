...
export const isElement = x => isInstanceOf(x, WIN.Element);
export const isDocument = x => isInstanceOf(x, WIN.Document);
export const isWindow = x => core.is(x) && x.window === x && isDocument(x.document);
export const isElDocWin = x => isElement(x) || isDocument(x) || isWindow(x);
...
export function onPrint(before, after) {
